...
/**
 * A Terminus instance represents a handler
 * for health checks and system signals
 */
class Terminus {
  /**
   * Initializes the Terminus instance by listening on
   * system signals and decorating the given server with
   * the given health checks.
   * @param {any} server The HTTP server instance which should get decorated with the given healthchecks
   * @param {object} options The options which configure the Termninus instance
   */
  ...
  async _onRequest (req, res, listener) {
    ...
    // If is not "our" health check
    if (!this._options.healthChecks[req.url]) {
  ...
  /**
   * Returns the status of the health check
   * of the given url
   * @param {string} url The url of the health check
   * @throws {HealthCheckNotFoundError} Will throw an error if the given url
   * has no registered health checks
   */
  async getHealthStatus (url) {
    ...
    if (this._state.isShuttingDown) {
      const healthRespose = this._getErrorResponse()
      if (healthRespose !== 'ok') {
    ...
    try {
      ...
      return this._getErrorResponse(error.causes || [error.message])
    }

    return this._getSuccesResponse(info)
  }
}
