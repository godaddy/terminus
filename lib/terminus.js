...
function decorateWithSignalHandler(server, signalToUse, options) {
  const { onSigterm, onSigint, onShutdown, timeout, logger } = options;
  let signalCleanUpFn = noopResolves;

  stoppable(server, timeout);

  const asyncServerStop = promisify(server.stop).bind(server);

  if (signalToUse === 'SIGTERM' && typeof onSigterm === 'function') {
    signalCleanUpFn = onSigterm;
  } else if (signalToUse === 'SIGINT' && typeof onSigint === 'function') {
    signalCleanUpFn = onSigint;
  }

  function cleanup() {
    asyncServerStop()
      .then(() => signalCleanUpFn())
      .then(() => onShutdown())
      .then(() => {
...
function terminus(server, options = {}) {
  ...
  const onSigint = options.onSigint || noopResolves;
