...
async function sendSuccess (res, { info, verbatim, statusOk, statusOkResponse }) {
  ...
  res.end(JSON.stringify(statusOkResponse))
}
...
function decorateWithHealthCheck (server, state, options) {
  const { healthChecks, logger, onSendFailureDuringShutdown, sendFailuresDuringShutdown, caseInsensitive, statusOk, statusOkResponse, statusError } = options
