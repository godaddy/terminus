...
describe('Terminus', () => {
  ...
  describe('supports onHealthcheck for the healthcheck route', () => {
    ...
    it('returns 503 once signal received', (done) => {
      ...
      execFile('node', ['lib/standalone-tests/terminus.onsignal.fail.js'], (_) => {
  ...
  describe('getHealthStatus', () => {
    it('returns the correct health status with info if everything is ok', async () => {
      ...
      expect(await terminus.getHealthStatus('/health')).to.deep.eq({ status: 'ok' })
    })
    it('returns the correct health status with info if everything is ok', async () => {
      const healthStatus = { ram: 8200 }
      const terminus = new Terminus(server, {
        healthChecks: {
          '/health': () => Promise.resolve(healthStatus)
        }
      })

      expect(await terminus.getHealthStatus('/health')).to.deep.eq({ status: 'ok', info: healthStatus })
    })
    it('returns the correct health status if a error is thrown', async () => {
      const errorMessage = 'Error message'
      const terminus = new Terminus(server, {
        healthChecks: {
          '/health': () => Promise.reject(new Error(errorMessage))
        }
      })

      expect(await terminus.getHealthStatus('/health')).to.deep.eq({ status: 'error', error: [errorMessage] })
    })

    it('returns the correct causes if a HealthCheckError is thrown', async () => {
      const errorMessage = 'Error message'
      const causes = ['Error1', 'Error2']
      const terminus = new Terminus(server, {
        healthChecks: {
          '/health': () => Promise.reject(new HealthCheckError(errorMessage, causes))
        }
      })

      expect(await terminus.getHealthStatus('/health')).to.deep.eq({ status: 'error', error: causes })
    })
  })

  describe('getHttpServer', () => {
    it('should return the identical http server', () => {
      const terminus = new Terminus(server)
      expect(terminus.getHttpServer(), server)
    })
  })
})
