...
describe('Terminus', () => {
  ...
  it('runs onSigint when getting SIGUSR2 signal', () => {
    try {
      execFileSync('node', ['lib/standalone-tests/terminus.onsigusr2.js'])
    } catch (ex) {
      expect(ex.stdout.toString().trim()).to.eql('on-sigusr2-runs')
      return
    }
  ...
  it('runs onShutdown after onSigusr2', () => {
    try {
      execFileSync('node', ['lib/standalone-tests/terminus.onshutdown.sigusr2.js'])
    } catch (ex) {
      expect(ex.stdout.toString().trim()).to.eql('on-sigusr2-runs\non-shutdown-runs')
      return
    }
